...
const PromoPage = () => {
  ...
  const [subtotal, setSubtotal] = useState(subTotalVa);
  const [discount, setDiscount] = useState(0);
  const [tax, setTax] = useState(0);
  const [total, setTotal] = useState(subTotalVa);


  const clearCoupon = () =>{
    setTotal(subTotalVa)
    setSubtotal(subTotalVa)
    setDiscount(0)
    setcouponCode()
